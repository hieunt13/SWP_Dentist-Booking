 ...
 * @author admin
 */
@WebServlet(name = "AdminDeleteDentistController", urlPatterns = {"/admin/AdminDeleteDentistController"})
public class AdminDeleteDentistController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(appointmnetDAO.checkDeleteCondition(ID) == false){
                request.setAttribute("ERROR", "Fail to delete (This dentist still appears in one or more appointments)");
            }else{
                AdminDentistManager dentistDAO = new AdminDentistManager();
                boolean check = dentistDAO.deleteDentist(ID);
                if (check){
                    url = SUCCESS;

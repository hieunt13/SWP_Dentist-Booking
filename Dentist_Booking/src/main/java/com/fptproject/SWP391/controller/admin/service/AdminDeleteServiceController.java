...
public class AdminDeleteServiceController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            AdminAppointmentDetailManager appointmnetDetailDAO = new AdminAppointmentDetailManager();
            if(appointmnetDetailDAO.checkDeleteCondition(ID) == false){
                request.setAttribute("ERROR", "Fail to delete (This service still appears in one or more appointments)");
            }else{
                AdminServiceManager serviceDAO = new AdminServiceManager();
                boolean check = serviceDAO.deleteService(ID);
                if (check){
                    url = SUCCESS;
            ...
            log("Error at AdminDeleteService Controller: " + e.toString());
        }finally{
